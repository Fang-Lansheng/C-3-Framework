...
class Trainer():
    ...
    def forward(self):
        ...
        for epoch in range(self.epoch, cfg.MAX_EPOCH):
            logging.info('> Epoch {:4d}/{:4d} '.format(epoch + 1, cfg.MAX_EPOCH) + '-' * 80)
            self.epoch = epoch
            ...
            # print('=' * 20)
            logging.info('    [train time: {:.2f}s]'.format(self.timer['train time'].diff))
            ...
            if epoch % cfg.VAL_FREQ == 0 or epoch > cfg.VAL_DENSE_START:
                ...
                logging.info('    [val time: {:.2f}s]'.format(self.timer['val time'].diff))
    ...
    def train(self):  # training for all datasets
        ...
        for i, data in enumerate(self.train_loader, 0):
            ...
            if (i + 1) % cfg.PRINT_FREQ == 0:
                ...
                logging.info('    [it: {:4d}] [loss: {:5.4f}] [lr: {:5.4f}] '
                             '[time: {:.2f}s] [cnt(gt/pred): {:.2f}/{:.2f}]'.
                             format(i + 1, loss.item(), self.optimizer.param_groups[0]['lr'] * 10000,
                                    self.timer['iter time'].diff,
                                    float(gt_map[0].sum().data / self.cfg_data.LOG_PARA),
                                    float(pred_map[0].sum().data)))
