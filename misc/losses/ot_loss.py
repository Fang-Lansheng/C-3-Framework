...
class OT_Loss(nn.Module):
    def __init__(self, c_size, stride, norm_cood, device, iter_in_ot=100, reg=10.0):
        super(OT_Loss, self).__init__()
        assert c_size % stride == 0

        self.c_size = c_size  # default: 256
        self.device = device  # default: cuda
        self.norm_cood = norm_cood  # default: 0
        ...
        if self.norm_cood:  # default: False
    ...
    def forward(self, normed_density, unnormed_density, points):
        ...
        for idx, im_points in enumerate(points):
            if len(im_points) > 0:
                ...
                P, log = sinkhorn(target_prob, source_prob, dis, self.reg, maxIter=self.iter_in_ot, log=True)
